...
import { compileContract } from '../scrypt_helper';
...
export const SEND_TX_FEE_RATE = 1.01
export const CHECK_TX_FEE_RATE = 1
...
export function loadOutDesc(fileName: string) {
...
export const genContract = function (name: string, use_desc = true, use_release = false) {
    if (use_desc) {
        ...
        if (use_release) {
            return buildContractClass(loadOutDesc(name + '_release_desc.json'))
        } else {
            return buildContractClass(loadOutDesc(name + '_debug_desc.json'))
        }
    }
    else {
...
export const getTxOutputProofInfo = function(tx: mvc.Transaction, outputIndex: number) {
    const info = getTxidInfo(tx)
    const output = tx.outputs[outputIndex]
    const res = {
        txHeader: info.txHeader,
        hashProof: info.outputHashProof,
        satoshis: output.satoshis,
        scriptHash: mvc.crypto.Hash.sha256(output.script.toBuffer()).toString('hex'),
    }
    return res
}

export const getTxOutputProof = function(tx: mvc.Transaction, outputIndex: number, emptyScriptHash: boolean = false) {
    const info = getTxidInfo(tx)
    const output = tx.outputs[outputIndex]
    const res = {
