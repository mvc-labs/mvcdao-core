...
export function getTokenName(script: Buffer) {
    return script.subarray(script.length - TOKEN_NAME_OFFSET, script.length - TOKEN_NAME_OFFSET + TOKEN_NAME_LEN)
}

export function getTokenSymbol(script: Buffer) {
    return script.subarray(script.length - TOKEN_SYMBOLE_OFFSET, script.length - TOKEN_SYMBOLE_OFFSET + TOKEN_SYMBOLE_LEN)
}

export function getTokenDecimal(script: Buffer) {
    return script.readUInt8(script.length - TOKEN_DECIMAL_OFFSET)
}
